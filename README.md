...
## Автотест (оркестратор реального пути, без моков крипто)
    python test_e2e.py             # сюиты plain + tls + wrongpin
    python test_e2e.py plain       # быстрая регрессия
Тест поднимает настоящий listener.py (управляет его stdin, проверяет stdout)
и настоящего lab-agent.exe — они общаются между собой, тест смотрит сквозной
путь. wrongpin — негативный: неверный пин => агент умирает rc=2, не ретраится.
Финал [RESULT] 19/19 (9 plain + 9 tls + 1 wrongpin).
